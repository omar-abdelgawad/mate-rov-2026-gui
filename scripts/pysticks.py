...
GRIPPER_LEFT_BUTTON = 4    # physical button 5
GRIPPER_RIGHT_BUTTON = 2   # physical button 3
...
class _GameController(Controller):
    ...
    def leftGripper(self):
        if self.joystick.get_button(GRIPPER_LEFT_BUTTON):
    ...
    def rightGripper(self):
        if self.joystick.get_button(GRIPPER_RIGHT_BUTTON):
...
# Different OSs have different names for the same controller
controllers = {
...
def get_controller():
    """Initialize pygame and find the connected controller."""
    if not pygame:
        return None
    pygame.display.init()
